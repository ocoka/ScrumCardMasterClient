    ...
    return {
      ...
      link: function oMaskedPostLink($scope, $element, $attrs, oValidateOn) {
        ...
        mask.split("").forEach(function(c, i) {
          "?" == c ?
            (len--, partialPosition = i) : defaults.definitions[c] ?
            (
          ...
          if (!$element.prop("readonly")) {
            ...
            oldVal = $element.val(), 8 === k || 46 === k || iPhone && 127 === k ?
              (
                ...
                shiftL(begin, end - 1),
                e.preventDefault(), (validatorState && validatorState.setState(false) && $scope.$apply())
              ) : 13 === k ?
              blurEvent.call(this, e) : 27 === k && (
        ...
        function checkVal(allow) {
          ...
          return allow ?
            writeBuffer() : partialPosition > lastMatch + 1 ?
            settings.autoclear || buffer.join("") === defaultBuffer ?
            ($element.val() && $element.val(""), clearBuffer(0, len)) : writeBuffer() : (writeBuffer(), $element.val($element.val().substring(0, lastMatch + 1))),
            partialPosition ?
            i : firstNonMaskPos;
        }
  ...
  .directive("oValidateon", ['$http', '$q', '$timeout', function($http, $q, $timeout) {
    ...
    return {
      ...
      controller: function oValidatorCtrl($scope, $element, $attrs, $http) {
        ...
        var _reviewRegex = /^[\d\+\-\(\)\\\/\.,!&A-Za-zА-ЯЁа-яё]{1,56}\s?(\s[\d\+\-\(\)\\\/\.,!&A-Za-zА-ЯЁа-яё]{1,56}){0,255}$/;
        ...
        var registeredValidators = {
          ...
          /* required to be filled (non empty) but defer the validation */
          reqdef: function oValidateRequiredDeffered($el, def) {
            return $q(function(resolve, reject) {
              $timeout(function() {
                resolve($el.val().length > 0);
              }, def);
            });
          },
          ...
          if (validated.$all) {
            $http.post(url, $element.serializeArray().reduce(function(p, n) {
              ...
              if (result.status == 409) {
                setSystemErrorStatus('Время действия сеанса истекло');
                setTimeout(function() {
        ...
        this.registerValidator = function oRegisterValidator(el, validatorName, clbValidator) {
          ...
          validated[el][validatorName] = {
            value: (this.validateOnload ? true : void(0))
          };
